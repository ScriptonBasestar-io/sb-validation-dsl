...
class ValidationBuilderTest {
    ...
    fun ValidationBuilderBase<String>.minLength(minValue: Int) =
        addConstraint("must have at least {0} characters".format(minValue.toString())) { it.length >= minValue }

    fun ValidationBuilderBase<String>.maxLength(minValue: Int) =
        addConstraint("must have at most {0} characters".format(minValue.toString())) { it.length <= minValue }

    fun ValidationBuilderBase<String>.matches(regex: Regex) =
        addConstraint("must have correct format") { it.contains(regex) }

    fun ValidationBuilderBase<String>.containsANumber() =
