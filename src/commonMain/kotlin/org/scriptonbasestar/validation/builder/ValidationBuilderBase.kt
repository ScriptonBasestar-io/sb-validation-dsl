...
abstract class ValidationBuilderBase<T> {
    ...
    internal abstract fun <R> onEachIterable(prop: KProperty1<T, Iterable<R>>, init: ValidationBuilderBase<R>.() -> Unit)
    @JvmName("onEachIterable")
    infix fun <R> KProperty1<T, Iterable<R>>.onEach(init: ValidationBuilderBase<R>.() -> Unit) = onEachIterable(this, init)
    ...
    internal abstract fun <K, V> onHashMap(prop: KProperty1<T, Map<K, V>>, init: ValidationBuilderBase<Map.Entry<K, V>>.() -> Unit)
    @JvmName("onHashMap")
    infix fun <K, V> KProperty1<T, Map<K, V>>.onHash(init: ValidationBuilderBase<Map.Entry<K, V>>.() -> Unit) = onHashMap(this, init)

//    abstract fun build(): Validation<T>

//    internal abstract fun <R> ValidationBuilder<R?>.ifValuePresent(init: ValidationBuilder<R>.() -> Unit)

    //    abstract infix fun <R> R?.ifKeyPresent(init: ValidationBuilder<R>.() -> Unit)
