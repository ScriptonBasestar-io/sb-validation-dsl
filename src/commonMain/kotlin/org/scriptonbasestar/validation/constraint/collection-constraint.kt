...
inline fun <reified T> ValidationBuilderBase<T>.maxItems(maxSize: Int): Constraint<T> = addConstraint(
    "must have at most {0} items".format(maxSize.toString()),
) {
    when (it) {
...
inline fun <reified T : Map<*, *>> ValidationBuilderBase<T>.maxProperties(maxSize: Int): Constraint<T> =
